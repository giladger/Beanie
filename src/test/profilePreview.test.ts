...
run('extracts per-step pressure and flow targets', () => {
...
run('renders an svg preview with pressure and flow paths when data exists', () => {
  const svg = renderProfilePreview({
    title: 'Stepped',
    steps: [{ pressure: 6, flow: 2 }, { pressure: 9, flow: 1.5 }] as unknown[]
  });
