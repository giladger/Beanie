 ...
 * preserve FIFO per batch, while the injected exact runner serializes the
 * fresh read and conditional update with other local mutations of that batch.
 */
export class DoseMutationReconciler {
  ...
  async enqueue(input: EnqueueDoseMutationInput): Promise<DoseMutationEnqueueResult> {
    ...
    try {
      const queued = await this.outbox.enqueue({
        ...
        aggregateKey: aggregateKey(input.batchId),
        payload: entry,
      ...
      const inserted = !this.volatileEnqueues.has(idempotencyKey);
      if (inserted &&
  ...
  private async reconcile(): Promise<void> {
    ...
    while (!this.disposed) {
      ...
      try {
        ...
        if (reconciled.saved) this.notifyBatchSaved(reconciled.saved, entry);
      } catch (error) {
  ...
  private async migrateLegacy(): Promise<void> {
    ...
    for (const entry of legacy) {
      ...
      await this.outbox.enqueue({
        ...
        kind: PENDING_DOSE_MUTATION_KIND,
        aggregateKey: aggregateKey(entry.batchId),
        payload: entry,
  ...
  private notifyBatchSaved(batch: BeanBatch, entry: PendingDose): void {
    try {
      this.deps.onBatchSaved(batch, entry);
    } catch (error) {
...
function aggregateKey(batchId: string): string {
